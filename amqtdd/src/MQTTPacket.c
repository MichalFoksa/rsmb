...
void* MQTTPacket_Factory(int socket, int* error)
{
	...
	*error = SOCKET_ERROR;  /* indicate whether an error occurred, or not */
	...
	{
		virtSocket = socket;
		socket = MQTTMPProtocol_get_real_socket(virtSocket);
	}
#endif
...
int MQTTPacket_send(int socket, Header header, char* buffer, int buflen)
{
	...
	if (socket < 0)
	{
		/* This is a virtual socket, which means an MP header needs to be
		 * prepended - so this means shuffling the buffers down one to
		 * make room.
		 *
		 * It also means a lookup of the Client so we can get the real
		 * socket to write to and the channel number to use in the header.
		 */
		Clients* client = MQTTMPProtocol_getClientForVirtualSocket(socket);
		int realSocket = client->actualSock;
		char *mpheader = malloc(3);
		char *ptr = mpheader+1;
		char *bufs[2] = {buf , buffer};
		int buflens[2] = {buf0len, buflen};
		int buffree[2] = {1, 1};

		mpheader[0] = MP_PACKET_MQTT;
		writeInt(&ptr,client->channel);
...
#endif
		rc = Socket_putdatas(socket, buf, buf0len, 1, &buffer, &buflen, &buffree);
 ...
 * @return the completion code (TCPSOCKET_COMPLETE etc)
 */
int MQTTPacket_sends(int socket, Header header, int count, char** buffers, int* buflens, int* buffree)
{
	...
	if (socket < 0)
	{
		...
		int realSocket = client->actualSock;
#if defined(WIN32)
		/* can't have variable length arrays on Windows */
		char *buffers2[6];
		int buflens2[6];
		int buffree2[6];
#else
		char *buffers2[count+1];
		int buflens2[count+1];
		int buffree2[count+1];
#endif
		int i;
		char *mpheader = malloc(3);
		char *ptr = mpheader+1;
		...
		for (i=0;i<count;i++)
		{
			...
			buflens2[i+1] = buflens[i];
			buffree2[i+1] = buffree[i];
		}
		rc = Socket_putdatas(realSocket, mpheader, 3, count+1, buffers2, buflens2, buffree2);
		if (rc != TCPSOCKET_INTERRUPTED)
			free(mpheader);
	}
	else
#endif
		rc = Socket_putdatas(socket, buf, buf0len, count, buffers, buflens, buffree);
	if (rc != TCPSOCKET_INTERRUPTED)
	  free(buf);
	FUNC_EXIT_RC(rc);
	return rc;
...
int MQTTPacket_send_publish(Publish* pack, int dup, int qos, int retained, int socket, char* clientID)
{
	...
	if (qos > 0)
	{
		...
		ptr = topiclen;
		writeInt(&ptr, lens[1]);
		rc = MQTTPacket_sends(socket, header, 4, bufs, lens, frees);
		if (rc != TCPSOCKET_INTERRUPTED)
			free(buf);
	}
	else
	{
		...
		int frees[3] = {1, 0, 0};
		writeInt(&ptr, lens[1]);
		rc = MQTTPacket_sends(socket, header, 3, bufs, lens, frees);
	}
	if (rc != TCPSOCKET_INTERRUPTED)
		free(topiclen);
	...
	{
		if (qos == 0)
			Log(LOG_PROTOCOL, 27, NULL, socket, clientID, retained, rc, min(20, pack->payloadlen),
					pack->payload);
		else
