...
void MQTTProtocol_storeQoS0(Clients* pubclient, Publish* publish)
{
	...
	if (pubclient->protocol == PROTOCOL_MQTT_MP)
		pw->socket = pubclient->actualSock;
	else
#endif
...
int MQTTProtocol_startPublishCommon(Clients* pubclient, Publish* publish, int qos, int retained)
{
	...
	if (rc == TCPSOCKET_COMPLETE && pubclient->outbound)
		time(&(pubclient->lastContact));
...
#if defined(MQTTMP)
	if (client->protocol == PROTOCOL_MQTT_MP)
		socket = client->actualSock;
#endif
	while (client->good && Socket_noPendingWrites(socket) && /* no point in starting a publish if a write is still pending */
		client->outboundMsgs->count < bstate->max_inflight_messages &&
		queuedMsgsCount(client) > 0 &&
		qos0count < bstate->max_inflight_messages
#if defined(MQTTS)
...
int MQTTProtocol_retry(time_t now, int doRetry)
{
	...
	while (current)
	{
		...
		if (client->protocol == PROTOCOL_MQTT_MP)
		{
			if (Socket_noPendingWrites(client->actualSock) == 0)
				continue;
		}
		else
#endif
